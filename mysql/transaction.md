...
- 一致性（Consistency）：保证事务提交后不会因为宕机等原因导致数据丢失；实现主要基于 redo log。数据库总是从一个一致性的状态转换到另外一个一致性状态
...
- 持久性（Durability）：一旦一个事务已经提交了,就算服务器崩溃,仍然需要在下次启动的时候自动恢复。事务追求的最终目标，一致性的实现既需要数据库层面的保障，也需要应用层面的保障。
...
InnoDB 默认的隔离级别是 RR（可重复读），后文会重点介绍 RR。需要注意的是，在 SQL 标准中，RR 是无法避免幻读问题的，但是 InnoDB 实现的 RR 避免了幻读问题。
